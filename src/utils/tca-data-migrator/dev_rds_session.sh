...
# for example:
#
#    $ sudo socat TCP-LISTEN:5432,reuseaddr,fork TCP4:topcoder-academy-dev.ci8xwsszszsw.us-east-1.rds.amazonaws.com:5432
# 
...
# 
# NOTE: If you have a tool like pgAdmin running that is ALREADY connected to the actual 
# localhost instance of the database server, simply stopping the local DB server will not
# force pgAdmin to relinquish the connection to port 5432. When you initialize this script
# to port forward over SSM, you should NOT see a "Connection accepted for session..." message
# until you initiate the connection to the remote database. Be sure to "Disconnect from server"
# on your true localhost server in pgAdmin before executing this script.
#
