...
- These caveats require us to provide the AWS Lambda runtime-compatible Prisma client binaries. The authoritative [schema.prisma](../../../../../prisma/schema.prisma) is shared with the main application. During package installation, `scripts/generate-prisma-client.js` derives a temporary service-specific schema from it and adds the Node.js 20+ Lambda target:
```
generator client {
...
```
The package `postinstall` hook generates the client automatically, so a clean install is sufficient:
```
$ npm ci
```
