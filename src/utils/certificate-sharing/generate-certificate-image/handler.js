...
exports.index = async (event) => {
  ...
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      await generateAndSaveImageAsync(
        browser,
        bucket,
        files[i],
        screenshotSelector
...
async function generateAndSaveImageAsync(
  browser,
  bucket,
  file,
  screenshotSelector
  ...
  console.info("Screenshot taken", file.url);

  console.info("Saving", file.path);

  await putObjectToS3Async(bucket, file.path, screenshot);
...
function validateParams(params) {
  validateRequired(["bucket", "files"], params);

  // verify the files param is a non-empty array
  if (!params.files?.[0]) {
    throw new Error(
      `The files param must be a non-empty array: ${params.files?.[0]}`
    );
  }

  params.files.forEach((file) => {
    validateRequired(["path", "url"], file);
  });
...
function validateRequired(required, params) {
  const requiredParam = required.find((param) => !params?.[param]);
